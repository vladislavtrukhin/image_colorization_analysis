# follow this tutorial for pytorch installation https://www.youtube.com/watch?v=GMSjDTU8Zlc
import os
import glob
import time
import numpy as np
...
class ECCVGenerator(BaseColor):
    def __init__(self, norm_layer=nn.BatchNorm2d):
        ...
        # model3=[nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1, bias=True),] #! changed stride from 1 to 2 here
        # model3+=[nn.ReLU(True),]
...
def lab_to_rgb(L, ab):
    ...
    # out_lab_orig = torch.cat((L, ab), dim=1)
...
def log_results(loss_meter_dict, file):
    for loss_name, loss_meter in loss_meter_dict.items():
        file.write(f"{loss_name}: {loss_meter.avg:.5f}")
        print(f"{loss_name}: {loss_meter.avg:.5f}")

def train_model(model, train_dl, epochs, display_every=498):
    text_file = open("./test_result", "w")
    ...
    for e in range(epochs):
        ...
        for data in tqdm(train_dl):
            ...
            if i % display_every == 0:
                text_file.write((f"\nEpoch {e+1}/{epochs}"))
                text_file.write((f"Iteration {i}/{len(train_dl)}"))
                print(f"\nEpoch {e+1}/{epochs}")
                print(f"Iteration {i}/{len(train_dl)}")
                log_results(loss_meter_dict, text_file) # function to print out the losses
